...
<script>
(function () {
    var mesaj = document.getElementById('mesaj'), uyari = document.getElementById('uyari'), onay = document.getElementById('onay'), tablo = document.getElementById('tablo');
    ...
    function durumDegistir(k, hedef, buton) {
    ...
    function onayKutusu(k, hedef) {
        ...
        var h=document.createElement('h2'); h.textContent=hedef==='ENABLED'?'Kampanyayı yayına al':'Kampanyayı duraklat'; kutu.appendChild(h);
        ...
        var girdi=null;
        if(hedef==='ENABLED'){var risk=document.createElement('p');risk.innerHTML='<strong>Dikkat:</strong> Bu işlem gerçek harcamayı başlatabilir.';kutu.appendChild(risk);var label=document.createElement('label');label.textContent='Onaylamak için kampanya adını yazın: ';girdi=document.createElement('input');girdi.type='text';girdi.autocomplete='off';girdi.setAttribute('aria-label','Kampanya adı onayı');label.appendChild(girdi);kutu.appendChild(label);}
        var vazgec=document.createElement('button');vazgec.type='button';vazgec.textContent='Vazgeç';var gonder=document.createElement('button');gonder.type='button';gonder.textContent=hedef==='ENABLED'?'Yayına al':'Duraklat';gonder.style.marginLeft='8px';
        if(girdi){gonder.disabled=true;girdi.oninput=function(){gonder.disabled=girdi.value!==s(k.name);};}
        kutu.appendChild(document.createElement('br'));kutu.appendChild(vazgec);kutu.appendChild(gonder);onay.appendChild(kutu);vazgec.onclick=function(){onay.replaceChildren();};gonder.onclick=function(){durumDegistir(k,hedef,gonder);};
    }
    ...
    function liste(ks) {
        ...
        ks.forEach(function(k){var tr=document.createElement('tr');[s(k.name),durum[k.status]||'Diğer ('+s(k.status)+')',kanal[k.advertising_channel_type]||s(k.advertising_channel_type),tl(k.budget_amount_micros),tarih(k.end_date_time)].forEach(function(x){var c=document.createElement('td');c.textContent=x;tr.appendChild(c);});var c=document.createElement('td');if(k.status!=='REMOVED'){var hedef=k.status==='ENABLED'?'PAUSED':'ENABLED';var b=document.createElement('button');b.type='button';b.textContent=hedef==='ENABLED'?'Yayına Al':'Duraklat';b.onclick=function(){onayKutusu(k,hedef);};c.appendChild(b);}else c.textContent='İşlem yok';tr.appendChild(c);body.appendChild(tr);});t.appendChild(body);tablo.appendChild(t);
    }
    fetch('api/index.php?islem=kampanya-listele',{credentials:'same-origin',headers:{Accept:'application/json'}}).then(function(r){return r.json();}).then(function(c){if(c.return!==1){bildir(c.mesaj||'Kampanyalar yüklenemedi.','alert-error');return;}mesaj.className='status-message';mesaj.textContent=c.mesaj||'';if(c.hesap&&c.hesap.manager){var u=document.createElement('p');u.className='alert alert-warning';u.textContent='Bağlı hesap Manager (MCC) hesabıdır; kampanya yönetilemez. Non-manager müşteri hesabı bağlayın.';uyari.appendChild(u);return;}window.ks=Array.isArray(c.kampanyalar)?c.kampanyalar:[];liste(window.ks);}).catch(function(){bildir('Kampanyalar yüklenemedi.','alert-error');});
}());
</script>
